...
class Curve():
    ...
    def show(self):
        ...
        for i in range(len(points)-1):
            ...
            # ellipse(nextPoint.x, nextPoint.y,20, 20)
    ...
    def traverse(self):
        self.__initTraverserPath()
        self.isTraversing = True
        self.node1.currentColor = self.node1.defaultColor
        
    
    def __initTraverserPath(self):
        density = 40
        self.traverserPath = []
        if self.node1 == self.node2:
            firstPoint = self.points[0]
            lastPoint = self.points[-1]
            path = functions.createPath(self.startX, self.startY, firstPoint.x, firstPoint.y)
            self.traverserPath.extend(path)
            self.traverserPath.extend(self.points)
            path = functions.createPath(lastPoint.x, lastPoint.y, self.endX, self.endY)
            self.traverserPath.extend(path)
        ...
        for i in range(1,density):
            ...
            finalX = self.node1.x + v.x + normal.x
            finalY = self.node1.y + v.y + normal.y
            self.traverserPath.append(PVector(finalX, finalY))
    
    def __showTraverser(self):
        if self.isTraversing:
            current = self.traverserPath[self.traverser]
            fill(0,255,0)
            ellipse(current.x, current.y, 20, 20)
            self.traverser += 1
            if self.traverser >= len(self.traverserPath):
                self.isTraversing = False
                self.traverser = 0
                self.node2.currentColor = (0,255,0)